...
class MultiModalDataset(Dataset):
    ...
    def read_npy(self,index):
        ...
        #print(str(filename+'.npy'))
        pth=os.path.join(str(self.vis_path.absolute()),str(filename+'.npy'))
        visit=np.load(pth)
        return visit
...
class DPN(nn.Module):
    def __init__(self, cfg):
        ...
        self.conv1 = nn.Conv2d(7, 64, kernel_size=3, stride=1, padding=1, bias=False) #卷积层
        ...
        #self.linear = nn.Linear(out_planes[2]+(num_blocks[2]+1)*dense_depth[2], 64) 
...
class MultiModalNet(nn.Module):
    def __init__(self, backbone1, backbone2, drop, pretrained=True):
        ...
        #print(img_model.last_linear.in_features)
        if drop > 0:
            self.img_fc = nn.Sequential(FCViewer(),
                                    nn.Dropout(drop),
                                    nn.ReLU(),
                                    # nn.Linear(img_model.last_linear.in_features,img_model.last_linear.in_features),
                                    # nn.Dropout(drop),
                                    # nn.ReLU(),
                                    nn.Linear(img_model.last_linear.in_features, 256))
